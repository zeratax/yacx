/**
 * Here, the creation and the compilation of a program souce are tested prior to
 * the creation of a kernel source.
 */
...
using yacx::Program, yacx::KernelArg, yacx::Options, yacx::Source, yacx::Header,
    yacx::Headers;

SCENARIO("Various programs are created and compiled under the following "
         "conditions.") {
  GIVEN("Options for the compilation of a kernel are important factors.") {
    // A1. Preparing the input for the creation and compilation of a source
    // program
    ...
    WHEN("A. Program is created under controlled conditions, ex. using default "
         "or equivalent GPU-architecture.") {
      options1.insertOptions(yacx::options::GpuArchitecture(3, 0));
      options1.insertOptions(yacx::options::FMAD(true));

      // B2. Controlled results using ex. default or equivalent GPU -
      // architecture.
    ...
    WHEN("C. Program is created using improper options values") {
      Options options2(
          {yacx::options::GpuArchitecture(8000, 0), yacx::options::FMAD(true)});

      // D2. Result using incompatible Option values.
      THEN("The program cannot be compiled.") {
        REQUIRE_THROWS_WITH(source1.program("cuda_add").compile(options2),
                            Catch::Contains(error_String));
      }
    }

    /**E1. Creating and compiling a program using multiple options with various
     * methods of insecting new option values.
     */
    WHEN("D. Program is created using various multiple options with various "
         "inserting methods.") {
      options_valueStr = "-restrict";

      options3.insertOptions(yacx::options::GpuArchitecture(3, 0),
    ...
    // F1. Creating and compiling a program using multiple and repeating
    // options.
    WHEN(
        "E. Program is created using various multiple and repeating options.") {
      options4.insertOptions(yacx::options::GpuArchitecture(3, 0),
                             yacx::options::FMAD(true));
      ...
      THEN("The program cannot be created and compiled.") {
        REQUIRE_THROWS_WITH(source1.program("cuda_add").compile(options4),
                            Catch::Contains(error_String));
      }
    }

    // G1. Creating and compiling a program using multiple and repeating
    // options.
    WHEN("F. Program is created using incorrect option syntaxes") {
      options5.insertOptions(yacx::options::GpuArchitecture(3, 0),
                             yacx::options::FMAD(true));
      options5.insert("-std=C_PLUS_PLUS_14");

      // G2. Result using various multiple options
      THEN("The program cannot be created and compiled.") {
        REQUIRE_THROWS_WITH(source1.program("cuda_add").compile(options5),
                            Catch::Contains(error_String));
      }
    }
  }
