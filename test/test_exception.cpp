...
TEST_CASE("NVRTC_SAFE_CALL", "[yacx::Exception]") {
  ...
  SECTION("NVRTC_SUCCESS"){
    try{
      ...
      FAIL("Correct nvrtcResultException, but it shouldn't be thrown an exception");
    }catch(std::exception &e){
      FAIL("Exception != nvrtcResultException, but it shouldn't be thrown an exception");
    }
  }
...
TEST_CASE("CUDA_SAFE_CALL", "[yacx::Exception]") {
  ...
  SECTION("CUDA_SUCCESS"){
    try{
      ...
      FAIL("Correct CUresultException, but it shouldn't be thrown an exception");
    }catch(std::exception &e){
      FAIL("Exception != CUresultException, but it shouldn't be thrown an exception");
    }
  }
