...
SCENARIO("Various sources of kernels are created and through them are created"
  ...
  GIVEN("Various sources of kernels with one or more functions.") {
    ...
    // B1. Use of more than one header, in which checking-mechanisms are
    // simulated.
    WHEN("More than one headers are used.") {
      ...
      THEN("The results are consistent with the given controlled output.") {
        ...
        sourceNew1.program("cuda_add_with_header")
            .compile()
            .configure(gridNew1, blockNew1)
            .launch(args);

        // B3. Comparing the results
        for (int i = 0; i < 5; i++)
          REQUIRE(hOut[i] == hostCompareOutput_1[i]);
      }
    }

    /**
     * C1. Checking for consistencies for running various functions in a
     * kernel-code
     */
    WHEN("More one functions in a kernel-code are used.") {
      ...
      THEN("The results are consistent with the given controlled output.") {
        /*
         * C3. Reusing the kernel-arguments arrays for testing of the coming
         * functions in a kernel-code by clearing the kernel-arguments array
         */
        args.clear();
        ...
        sourceNew2.program("cuda_add_five")
            .compile()
            .configure(gridNew2, blockNew2)
            .launch(args);

        // C4. Compiling another function in a kernel-code.
        /*
         * C5. Reusing the kernel-arguments arrays for testing of the coming
         * functions in a kernel-code by clearing the kernel-arguments array
         */
        args.clear();
        ...
        sourceNew2.program("cuda_add_normal")
            .compile()
            .configure(gridNew2, blockNew2)
            .launch(args);

        // C6. Comparing the results for the use of multiple functions in a
        // kernel-code
        for (int i = 0; i < 5; i++)
          REQUIRE(hOut_Multiply1[i] == hostCompareOutput_1[i]);
        for (int i = 0; i < 5; i++)
          REQUIRE(hOut_Multiply2[i] == hostCompareOutput_Controlled[i]);
      }
    }

    // D1. Checking for the validation of declaring functions
    WHEN("The naming of the kernel-compilation function is not consistent to "
         "that of that function of a kernel-code.") {
      /*
       * D2. Reusing the kernel-arguments arrays for testing of the coming
       * functions in a kernel-code by learing the kernel-arguments array
       */
      args.clear();
      ...
      program from kernel - source*/
      THEN(
          "The results for naming functions in a kernel-code are consistent.") {
        ...
        /*
         * D4. Checking the possibility of calling a kernel compilation function
         * using kernel function conventions.
         */
        REQUIRE_THROWS_WITH(sourceNew3.program("this_function_does_not_exist")
                                .compile()
                                .configure(gridNew3, blockNew3)
                                .launch(args),
                            Catch::Contains(yacx::detail::whichError(error)));
      }
    }
